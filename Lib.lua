...
local function sendRequest(url)
	...
	if not tab then
		print('Error while parsing JSON', code)
		print('Data:', dat)
...
function api.getMe(TOKEN)

	local url = BASE_URL .. TOKEN .. '/getMe'

	return sendRequest(url)

end

function api.getUpdates(TOKEN, offset)

	local url = BASE_URL .. TOKEN .. '/getUpdates?timeout=20'
...
function api.unbanChatMember(TOKEN, chat_id, user_id)
	
	local url = BASE_URL .. TOKEN .. '/unbanChatMember?chat_id=' .. chat_id .. '&user_id=' .. user_id

	return sendRequest(url)
end

function api.kickChatMember(TOKEN, chat_id, user_id)
	
	local url = BASE_URL .. TOKEN .. '/kickChatMember?chat_id=' .. chat_id .. '&user_id=' .. user_id
...
function api.banUser(TOKEN, chat_id, user_id)
	
	local res, code = api.kickChatMember(TOKEN, chat_id, user_id) --try to kick. "code" is already specific
...
function api.kickUser(TOKEN, chat_id, user_id)
	
	local res, code = api.kickChatMember(TOKEN, chat_id, user_id) --try to kick
	
	if res then --if the user has been kicked, then...
		--unban
		api.unbanChatMember(TOKEN, chat_id, user_id)
		api.unbanChatMember(TOKEN, chat_id, user_id)
		api.unbanChatMember(TOKEN, chat_id, user_id)
		return res
	else
...
function api.unbanUser(TOKEN, chat_id, user_id)
	
	local res, code = api.unbanChatMember(TOKEN, chat_id, user_id)
	return true
end

function api.getChat(TOKEN, chat_id)
	
	local url = BASE_URL .. TOKEN .. '/getChat?chat_id=' .. chat_id
	
	return sendRequest(url)
	
end

function api.getChatAdministrators(TOKEN, chat_id)
	
	local url = BASE_URL .. TOKEN .. '/getChatAdministrators?chat_id=' .. chat_id
...
function api.getChatMembersCount(TOKEN, chat_id)
	
	local url = BASE_URL .. TOKEN .. '/getChatMembersCount?chat_id=' .. chat_id
	
	return sendRequest(url)
	
end

function api.getChatMember(TOKEN, chat_id, user_id)
	
	local url = BASE_URL .. TOKEN .. '/getChatMember?chat_id=' .. chat_id .. '&user_id=' .. user_id
...
function api.leaveChat(TOKEN, chat_id)
	
	local url = BASE_URL .. TOKEN .. '/leaveChat?chat_id=' .. chat_id
...
function api.sendMessage(TOKEN, chat_id, text, parse_mode, reply_markup, reply_to_message_id, link_preview)	
	local url = BASE_URL .. TOKEN .. '/sendMessage?chat_id=' .. chat_id .. '&text=' .. URL.escape(text)
...
function api.editMessageText(TOKEN, chat_id, message_id, text, parse_mode, keyboard)

	local url = BASE_URL .. TOKEN .. '/editMessageText?chat_id=' .. chat_id .. '&message_id='..message_id..'&text=' .. URL.escape(text)
...
function api.editMessageTextII(TOKEN, inline_message_id, text, parse_mode, keyboard, DP)

	local url = BASE_URL .. TOKEN .. '/editMessageText?inline_message_id=' .. inline_message_id .. '&text=' .. URL.escape(text)
...
function api.editMarkup(TOKEN, chat_id, message_id, reply_markup)
	
	local url = BASE_URL .. TOKEN .. '/editMessageReplyMarkup?chat_id=' .. chat_id ..
...
function api.answerCallbackQuery(TOKEN, callback_query_id, text, show_alert, cache_time)
	
	local url = BASE_URL .. TOKEN .. '/answerCallbackQuery?callback_query_id=' .. callback_query_id .. '&text=' .. URL.escape(text)
...
function api.sendChatAction(TOKEN, chat_id, action)
 -- Support actions are typing, upload_photo, record_video, upload_video, record_audio, upload_audio, upload_document, find_location
	local url = BASE_URL .. TOKEN .. '/sendChatAction?chat_id=' .. chat_id .. '&action=' .. action
	return sendRequest(url)

end

function api.sendLocation(TOKEN, chat_id, latitude, longitude, reply_to_message_id)

	local url = BASE_URL .. TOKEN .. '/sendLocation?chat_id=' .. chat_id .. '&latitude=' .. latitude .. '&longitude=' .. longitude
...
function api.forwardMessage(TOKEN, chat_id, from_chat_id, message_id)

	local url = BASE_URL .. TOKEN .. '/forwardMessage?chat_id=' .. chat_id .. '&from_chat_id=' .. from_chat_id .. '&message_id=' .. message_id
...
function api.getFile(TOKEN, file_id)
	
	local url = BASE_URL .. TOKEN .. '/getFile?file_id='..file_id
	
	return sendRequest(url)
	
end

function api.answerInlineQuery(TOKEN, inline_query_id, results, cache_time, is_personal, switch_pm_text, switch_pm_parameter)
	
	local url = BASE_URL .. TOKEN .. '/answerInlineQuery?inline_query_id='..inline_query_id..'&results='..results--JSON.encode(results)
...
function api.sendMediaId(TOKEN, chat_id, file_id, media, reply_to_message_id)
...
function api.sendPhotoId(TOKEN, chat_id, file_id, reply_to_message_id, caption)
	
	local url = BASE_URL .. TOKEN .. '/sendPhoto?chat_id=' .. chat_id .. '&photo=' .. file_id
...
function api.sendVideoId(TOKEN, chat_id, file_id, reply_to_message_id, caption)
	
	local url = BASE_URL .. TOKEN .. '/sendVideo?chat_id=' .. chat_id .. '&photo=' .. file_id
...
function api.sendDocumentId(TOKEN, chat_id, file_id, reply_to_message_id, caption)
	
	local url = BASE_URL .. TOKEN .. '/sendDocument?chat_id=' .. chat_id .. '&document=' .. file_id
...
function api.sendPhoto(TOKEN, chat_id, photo, caption, reply_to_message_id)

	local url = BASE_URL .. TOKEN .. '/sendPhoto'
...
function api.sendDocument(TOKEN, chat_id, document, reply_to_message_id, caption)

	local url = BASE_URL .. TOKEN .. '/sendDocument'
...
function api.sendSticker(TOKEN, chat_id, sticker, reply_to_message_id)

	local url = BASE_URL .. TOKEN .. '/sendSticker'
...
function api.sendStickerId(TOKEN, chat_id, file_id, reply_to_message_id)
	
	local url = BASE_URL .. TOKEN .. '/sendSticker?chat_id=' .. chat_id .. '&sticker=' .. file_id
...
function api.sendAudio(TOKEN, chat_id, audio, reply_to_message_id, duration, performer, title)

	local url = BASE_URL .. TOKEN .. '/sendAudio'
...
function api.sendVideo(TOKEN, chat_id, video, reply_to_message_id, duration, performer, title)

	local url = BASE_URL .. TOKEN .. '/sendVideo'
...
function api.sendVoice(TOKEN, chat_id, voice, reply_to_message_id)

	local url = BASE_URL .. TOKEN .. '/sendVoice'
...
function api.getUserProfilePhotos(TOKEN, user_id, offset, limit)
	local url = BASE_URL .. TOKEN .. '/getUserProfilePhotos?user_id='..user_id
	if offset then
		url = url .. '&offset='..offset
	end
	if limit then
...
M.sendForwarded = sendForwarded
	...
	local function sendMention(chat_id, user_id, msg_id, text, offset, length)
					  tdcli_function ({
					        ID = "SendMessage",
					        chat_id_ = chat_id,
  				...
  				local function send_Inline(Bot_Username, chat_ID, msg_ID, TEXT, KeyID)
  					searchPublicChat(Bot_Username, function (A, D)
  							if D.type_.user_.type_.ID == "UserTypeBot" then
  								userid = D.id_ 
  								tdcli_function(
  									{
  										ID = 'GetInlineQueryResults',
  									...
  									function (ARG, DTA)
  										 if DTA.results_ and DTA.results_[KeyID] then
  										 	tdcli_function(
  										 		{
  										 			ID = "SendInlineQueryResultMessage",
  										 			chat_id_ = chat_ID,
  										 			reply_to_message_id_ = msg_ID or 0,
  										 			disable_notification_ = 0,
  										 			from_background_ = 1,
  										 			query_id_ = DTA.inline_query_id_,
  										 			result_id_ = DTA.results_[KeyID].id_
  										 		},
  										 	dl_cb,
  										 	nil
  										 	)
  		...
  		M.sendInline = send_Inline
  		M.sendMention = sendMention
return {api = api, cli = M}
